...
const JPN225_CANDIDATES = [SYMBOLS.JPN225, "N225", "NI225", "JP225"].filter((v,i,a)=>v && a.indexOf(v)===i);
...
async function signalFor(key){
  const interval=process.env.MARKET_INTERVAL||"15min";
  const candidates=key==="JPN225"?JPN225_CANDIDATES:[SYMBOLS[key]];
  let lastError=null;
  for(const symbol of candidates){
    try{
      const data=await td("/time_series",{symbol,interval,outputsize:260,timezone:"UTC"});
      if(!data.values?.length) throw new Error(`${key}: no time-series values for ${symbol}`);
      ...
      sig.sourceSymbol=symbol;
      if(key==="JPN225" && symbol!==SYMBOLS.JPN225) sig.symbolFallback=true;
      return sig;
    }catch(e){lastError=e;}
  }
  throw new Error(`${key}: no supported Twelve Data symbol found. Tried ${candidates.join(", ")}. ${lastError?.message||""}`.trim());
}

app.get("/api/health",(req,res)=>res.json({ok:true,time:new Date().toISOString(),marketProvider:TD_KEY?"twelve-data":"missing",economicProvider:FINNHUB_KEY?"finnhub":FRED_KEY?"fred":"missing",configured:{twelveData:Boolean(TD_KEY),finnhub:Boolean(FINNHUB_KEY),fred:Boolean(FRED_KEY)}}));
...
app.get("/api/signals",async(req,res)=>{
  ...
  results.forEach((r,i)=>{
    if(r.status==="fulfilled") out[entries[i]]=r.value;
    else out[entries[i]]={direction:"WAIT",price:null,entry:null,stopLoss:null,tp1:null,tp2:null,indicators:{},error:true,reason:r.reason?.message||"Market data unavailable",timestamp:new Date().toISOString(),sourceSymbol:SYMBOLS[entries[i]]};
  });
  res.json(out);
});
...
async function finnhubCalendar(){
  ...
  if(!r.ok || j.error) throw new Error(j.error||`Finnhub ${r.status}`);
  return (j.economicCalendar||[]).filter(e=>String(e.country||"").toUpperCase()==="US").map(e=>({name:e.event,country:e.country,impact:e.impact||"HIGH",date:e.time,actual:e.actual,forecast:e.estimate,previous:e.prev,type:String(e.event||"").toUpperCase().includes("FOMC")?"FOMC":undefined,unit:e.unit}));
}
...
app.get("/api/fed",async(req,res)=>{
  if(!FRED_KEY) return res.json({rate:null,source:"not-configured"});
  try{
    const u=new URL("https://api.stlouisfed.org/fred/series/observations");
    u.searchParams.set("series_id","DFF");u.searchParams.set("api_key",FRED_KEY);u.searchParams.set("file_type","json");u.searchParams.set("sort_order","desc");u.searchParams.set("limit","1");
    const r=await fetch(u);const j=await r.json();
    if(!r.ok || j.error_code) throw new Error(j.error_message||`FRED ${r.status}`);
    const v=j.observations?.[0]?.value;
    res.json({rate:v&&v!=="."?Number(v):null,source:"fred",asOf:j.observations?.[0]?.date||null});
  }catch(e){res.status(502).json({error:e.message})}
});
