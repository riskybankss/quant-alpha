...
This package converts the supplied dashboard into a deployment-ready full-stack application. This revision fixes same-origin frontend API calls, improves provider error handling, and retries common Nikkei 225 symbol variants.
...
Twelve Data documents `/time_series` for OHLC historical data and reference-data endpoints for supported instruments. Nikkei 225 is listed as `N225`; this package also retries `NI225` and `JP225` if the configured symbol is rejected. See https://twelvedata.com/docs.
...
- CORS configuration
- API error handling
- Same-origin API integration (no frontend API URL required)
- JPN225 symbol fallback handling
